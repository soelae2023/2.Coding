...
namespace SimpleApISystem.Middlewares
{
    public class LoggingMiddleware
    {
        ...
        private async Task LogResponse(HttpContext context, MemoryStream responseBody, Stream originalResponseBody)
        {
            ...
            responseContent.AppendLine("=== Response headers ===");
        ...
        private async Task LogRequest(HttpContext context)
        {
            ...
            requestContent.AppendLine("=== Http Request Info ===");
            requestContent.AppendLine($"Http Method = {context.Request.Method.ToUpper()}");
            requestContent.AppendLine($"Request Url = {context.Request.Path}");

            requestContent.AppendLine("=== Http headers ===");
            foreach (var (headerKey, headerValue) in context.Request.Headers)
            {
                requestContent.AppendLine($"header = {headerKey}    value = {headerValue}");
            }

            requestContent.AppendLine("=== Http Request body ===");
